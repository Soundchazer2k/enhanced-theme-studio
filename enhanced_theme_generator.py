...
def ensure_wcag_compliant(fg_color, bg_color, min_ratio=4.5, preserve_character=True):
    ...
    """
    current_ratio = contrast_ratio(fg_color, bg_color)
    
    if current_ratio >= min_ratio:
        return bg_color  # Already compliant
    
    # Get the original color components
    r, g, b = hex_to_rgb(bg_color)
    h, l, s = rgb_to_hls(r, g, b)
    
    # Determine if we need to darken or lighten based on relative luminance
    fg_lum = relative_luminance(*hex_to_rgb(fg_color))
    bg_lum = relative_luminance(*hex_to_rgb(bg_color))
    
    darken = fg_lum < bg_lum  # If foreground is darker, darken background
    
    # Store original values to preserve color character
    original_h, original_s = h, s
    
    # First try adjusting just luminance to maintain color character
    if preserve_character:
        adjusted_bg = bg_color
        step = 0.05  # Start with a 5% adjustment
        max_steps = 12
        
        for _ in range(max_steps):
            if darken:
                l = max(0.05, l - step)  # Avoid pure black
            else:
                l = min(0.95, l + step)  # Avoid pure white
                
            adjusted_bg = hls_to_hex(original_h, l, original_s)
            new_ratio = contrast_ratio(fg_color, adjusted_bg)
    ...
    # If we couldn't preserve character, fall back to more aggressive adjustment
    # For extreme cases, we may need to go towards black or white
    if darken:
        return hls_to_hex(original_h, 0.05, original_s)  # Very dark
    else:
        return hls_to_hex(original_h, 0.95, original_s)  # Very light
...
class ColorPickerWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.hex_input.setValidator(QtGui.QRegularExpressionValidator(QRegularExpression(r'#?[0-9A-Fa-f]{0,8}')))
    ...
    def open_color_dialog(self):
        color = QtWidgets.QColorDialog.getColor(QtGui.QColor(self.hex_input.text()), self)
        if color.isValid():
...
class EnhancedThemeGenerator(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.random_color_btn.setIcon(self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_BrowserReload))
        ...
        self.preserve_character_check = QtWidgets.QCheckBox("Preserve color character when adjusting")
        self.preserve_character_check.setChecked(True)
        self.preserve_character_check.setToolTip("When checked, tries to maintain the color's hue and saturation while adjusting luminance")
        ...
        self.colorblind_combo.addItems(["Normal Vision", "Protanopia", "Deuteranopia", "Tritanopia", "Grayscale"])
        ...
        self.auto_adjust_dark = QtWidgets.QCheckBox("Auto-create matching dark/light variant")
        self.auto_adjust_dark.setChecked(True)
        self.auto_adjust_dark.setToolTip("Automatically generates a matching variant when switching modes")
        ...
        generate_btn.setIcon(self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_BrowserReload))
        generate_btn.setToolTip("Generate a new color palette based on current settings")
    ...
    def create_advanced_widgets_preview(self, container):
        ...
        self.card_content = QtWidgets.QLabel("This is sample content for a card component that demonstrates how colors work in a common UI pattern.")
    ...
    def on_scheme_changed(self, scheme):
        if scheme in ("Split-Complementary", "Triadic", "Tetradic"):
            ...
            self.statusBar().showMessage(f"Note: {scheme} scheme uses fixed count of {count} colors")
        else:
            self.statusBar().showMessage("Ready")
        self.update_palette()
    ...
    def update_palette(self):
        ...
        for i, hexcol in enumerate(cols):
            ...
            if self.wcag_check.isChecked() and wcag not in ["AAA"] and ratio < min_ratio:
                adjusted_col = ensure_wcag_compliant(fg, hexcol, min_ratio, preserve_character)
            ...
            if adjusted_col != hexcol:
                adjusted_swatch.setToolTip(f"Adjusted from {hexcol} for {adjusted_wcag} compliance")
    ...
    def update_preview_components(self, palette):
        ...
        if simulation_type != "Normal Vision":
            ...
            self.set_status(f"Simulating {simulation_type} vision", 0)  # Don't auto-clear this message
        else:
        ...
        if hasattr(self, 'color_wheel'):
            self.color_wheel.set_colors(
                ...
                simulate_colorblindness(self.color_picker.hex_input.text(), simulation_type)
            )
    ...
    def export_theme(self):
        ...
        include_semantic = QtWidgets.QCheckBox("Include semantic naming (primary, secondary, etc.)")
        ...
        if format_type == "QSS (Qt Style Sheet)":
            content = self.export_as_qss(palette, foregrounds, with_comments, with_semantic, theme_name_value)
            file_filter = "QSS files (*.qss)"
        elif format_type == "CSS Variables":
            content = self.export_as_css(palette, foregrounds, with_comments, with_semantic, theme_name_value)
            file_filter = "CSS files (*.css)"
        elif format_type == "Tailwind Config":
            content = self.export_as_tailwind(palette, with_comments, with_semantic, theme_name_value)
            file_filter = "JavaScript files (*.js)"
        elif format_type == "JSON":
            content = self.export_as_json(palette, foregrounds, with_comments, with_semantic, include_both_modes, theme_name_value)
        ...
        if path:
            with open(path, 'w') as f:
                f.write(content)
            QtWidgets.QMessageBox.information(self, "Export", f"Theme '{theme_name_value}' saved to {path}")
    
    def export_as_qss(self, palette, foregrounds, with_comments=True, with_semantic=True, theme_name="Theme"):
        ...
        if with_comments:
            ...
            header += f" * Generated with Enhanced Theme Studio\n"
    ...
    def export_as_css(self, palette, foregrounds, with_comments=True, with_semantic=True, theme_name="Theme"):
    ...
    def export_as_tailwind(self, palette, with_comments=True, with_semantic=True, theme_name="Theme"):
        ...
        if with_semantic and len(palette) >= 3:
            ...
            content += f"        background: '{palette[3 if len(palette) > 3 else 0]}',\n"
    ...
    def export_as_json(self, palette, foregrounds, with_comments=True, with_semantic=True, include_both_modes=True, theme_name="Theme"):
        ...
        theme = {
            ...
            'metadata': {
                ...
                'scheme': self.scheme_combo.currentText(),
                'wcag_compliance': self.wcag_level.currentText() if self.wcag_check.isChecked() else 'Off',
        ...
        if with_semantic and len(palette) >= 3:
            theme['semantic'] = {
                ...
                'text_secondary': foregrounds[1] if len(foregrounds) > 1 else foregrounds[0],
                'text_accent': foregrounds[2] if len(foregrounds) > 2 else foregrounds[0],
                'text_background': foregrounds[3] if len(foregrounds) > 3 else '#000000'
            }
        
        # Add theme mode variants if requested
        if include_both_modes and hasattr(self, 'light_palette') and hasattr(self, 'dark_palette'):
            theme['light_mode'] = {
...
"""
            ...
            for i, p in enumerate(self.saved_palettes):
                if p["name"] == palette_name:
                    ...
                    if reply == QtWidgets.QMessageBox.StandardButton.Yes:
                        ...
                        self.statusBar().showMessage(f"Palette '{palette_name}' updated")
                        return
                    else:
                        return
    ...
    def load_palette(self):
        ...
        for palette in self.saved_palettes:
            ...
            for color in palette["colors"]:
                ...
                swatch.setStyleSheet(f"background-color: {color}; border: 1px solid #CCCCCC;")
        ...
        # Show dialog
        if dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted and list_widget.currentItem():
    ...
    def delete_palette(self, list_widget, dialog):
        ...
        if reply == QtWidgets.QMessageBox.StandardButton.Yes:
            ...
            if not self.saved_palettes:
                ...
                QtWidgets.QMessageBox.information(
                    self, "No Saved Palettes", 
                    "You don't have any saved palettes left."
                )
    ...
    def generate_new_palette(self):
        ...
        # Show message
        self.set_status(f"New palette generated! Base color slightly varied from {old_base} to {new_base}")
    ...
    def create_dark_variant(self):
        ...
        # Start with a dark background (usually 4th color if available)
        dark_palette = []
        ...
        for i, color in enumerate(self.light_palette):
            ...
            # For dark mode, we generally:
            # 1. Invert lightness for most colors
            # 2. Make sure background colors are darker
            # 3. Make sure text/foreground colors are lighter
            
            if i == 0:  # Primary color - keep hue, adjust lightness
                new_l = 0.7 if l < 0.5 else 0.3
            elif i == 1:  # Secondary color - keep hue, adjust lightness
                new_l = 0.6 if l < 0.5 else 0.4
            elif i == 2:  # Accent color - keep hue and saturation, increase lightness
                new_l = 0.8 if l < 0.5 else 0.75
            elif i == 3:  # Background color - make it dark
                new_l = 0.1
                s = min(s, 0.3)  # Reduce saturation for background
            else:
                # Invert lightness for other colors
                new_l = 1.0 - l
        ...
        if self.wcag_check.isChecked():
            # Create adjusted version of dark palette for accessibility
            adjusted_palette = []
            min_ratio = 7.0 if "AAA" in self.wcag_level.currentText() else 4.5
            preserve_character = self.preserve_character_check.isChecked()
            ...
            for color in self.dark_palette:
                ...
                if wcag not in ["AAA"] and ratio < min_ratio:
                    adjusted_color = ensure_wcag_compliant(fg, color, min_ratio, preserve_character)
                else:
            ...
            self.update_preview_components(self.dark_palette)

    def create_light_variant(self):
        """Create a light mode variant of the current palette"""
        if not hasattr(self, 'light_palette'):
            # If we don't have a stored light palette, regenerate from base
            self.update_palette()
            return
        
        # Use the stored light palette
        ...
        # Update widget preview
        if self.wcag_check.isChecked():
            # Create adjusted version of light palette for accessibility
            adjusted_palette = []
            min_ratio = 7.0 if "AAA" in self.wcag_level.currentText() else 4.5
            preserve_character = self.preserve_character_check.isChecked()
            
            for color in self.light_palette:
                fg = pick_foreground(color)
                ratio = contrast_ratio(color, fg)
                wcag = get_wcag_level(ratio)
            ...
            self.update_preview_components(self.light_palette)
...
class ColorWheelWidget(QtWidgets.QWidget):
    ...
    def draw_palette_markers(self, painter, center_x, center_y, radius):
        ...
        painter.setPen(QtGui.QPen(QtGui.QColor(100, 100, 100, 150), 2, Qt.PenStyle.DashLine))
        ...
        for i, color in enumerate(self.colors):
            ...
            # Draw color number
            painter.setPen(QtGui.QPen(QtGui.QColor("#FFFFFF" if l < 0.5 else "#000000")))
            font = painter.font()
            font.setPointSize(8)
